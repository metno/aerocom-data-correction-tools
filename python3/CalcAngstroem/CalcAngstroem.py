...
def CalcAngstroemLL(AODLow, AODHigh, LambdaLow, LambdaHigh, DebugFlag=False):
	"""
...
def CalcAngstroem(InData, DebugFlag=False, VerboseFlag=False):
	"""
	...
	for dict_Data in InData:
		...
		if LambdaLow < 0.:
			if 'od550aer' in dict_Data.keys():
				AODLow=dict_Data['od550aer']
				OutData=dict_Data
				del OutData['od550aer']
				LambdaLow=.55
				VarLowStr='od550aer'
			elif 'od440aer' in dict_Data.keys():
				AODLow=dict_Data['od440aer']
				OutData=dict_Data
				del OutData['od440aer']
				LambdaLow=.44
				VarLowStr='od440aer'
		...
		if LambdaHigh < 0.:
			if 'od825aer' in dict_Data.keys():
				AODHigh=dict_Data['od825aer']
				del dict_Data['od825aer']
				LambdaHigh=.825
				VarHighStr='od825aer'
			elif 'od865aer' in dict_Data.keys():
				AODHigh=dict_Data['od865aer']
				del dict_Data['od865aer']
				LambdaHigh=.865
				VarHighStr='od865aer'
			elif 'od870aer' in dict_Data.keys():
				AODHigh=dict_Data['od870aer']
				del dict_Data['od870aer']
				LambdaHigh=.87
				VarHighStr='od870aer'
...
def CalcAngstroemForDir(ModelDir, Outdir=None, DebugFlag=False, VerboseFlag=False):
	"""
	Routine to calculate the angstroem exponent from the data 
	...
	for Year in CommonYears:
		for TSName in TimeStepStringsToWorkOn:
			...
			if len(FilesFound) >= 2:
				...
				OutData, ReturnFlag=CalcAngstroem(InData, DebugFlag=False, VerboseFlag=False)
				if ReturnFlag == 0:
...
if __name__ == '__main__':
	...
	dict_Param={}
	parser = argparse.ArgumentParser(description='CalcAngstroem: Calculate angstroem exponent and save it into a new netcdf file\n\n')
...
#
	...
	FilesWritten=CalcAngstroemForDir(dict_Param['modeldir'], Outdir=dict_Param['outdir'], DebugFlag=dict_Param['debug'], VerboseFlag=dict_Param['info'])
